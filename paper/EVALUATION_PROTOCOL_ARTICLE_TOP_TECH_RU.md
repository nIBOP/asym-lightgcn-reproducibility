...
AsymLightGCN в этой статье не рассматривается как самостоятельный предмет архитектурного анализа; модель используется как проверяемый графово-семантический метод. Минимально необходимое определение следующее: LightGCN-часть формирует пользовательский вектор \(e_u\) и коллаборативный вектор объекта \(g_i\), семантический вектор \(s_i\) проецируется в латентное пространство \(z_i=P_s(s_i)\), а итоговое объектное представление задается смесью \(\tilde e_i=(1-w_i)g_i+w_i z_i\), где вес \(w_i\) убывает с ростом степени объекта \(c_i\). Поэтому редкие объекты получают большую семантическую поддержку, а популярные остаются ближе к LightGCN. Обучение сочетает BPR-потерю и семантическую регуляризацию; при ранжировании используется \(\hat y_{ui}=e_u^\top \tilde e_i/(\|\tilde e_i\|^\gamma+\varepsilon)\), \(\gamma=0{,}25\). Полные формулы функции потерь, псевдокод, конфигурации, split-протокол, скрипты построения эмбеддингов и статистическая обработка вынесены в публичный репозиторий воспроизводимости: https://github.com/nIBOP/asym-lightgcn-reproducibility. Исходные наборы данных не включаются в архив реализации; они восстанавливаются по открытым страницам источников с сохранением указанных фильтров и контрольных статистик срезов.
...
Все модели сравнивались в едином режиме полного ранжирования: из выдачи исключались объекты из обучающей истории пользователя, после чего качество оценивалось по Recall@10, NDCG@10, Recall@50 и NDCG@50. Диагностический протокол включает проверку разбиения, расчет основных метрик для обучаемых моделей, семантический и популярностный контроли, стратификацию объектов по популярности и проверку устойчивости ключевых сравнений.
...
Семантический и популярностный контроли нужны не как дополнительные конкурентные модели, а как проверка возможного объяснения результата. Если разность между обучаемой моделью и контролем включает ноль по доверительному интервалу либо относительный зазор не превышает 5%, результат трактуется как диагностически близкий к контролю. Стратификация по популярности объектов проверяет, сопровождается ли изменение общей метрики релевантными попаданиями в редком сегменте каталога.

Итоговая интерпретация строится вокруг диагностического режима.
...
Этот случай объясняет, почему сравнения только между обучаемыми моделями недостаточно: AsymLightGCN превосходит LightGCN, но уступает простой популярностной стратегии. По результатам Yelp нельзя утверждать, что модель выходит за пределы популярностной выдачи; здесь прежде всего проявляется популярностное смещение.
