...
В исследованиях рекомендательных систем новые модели часто сравнивают по агрегированным метрикам ранжирования. Такой подход удобен, но не всегда объясняет природу полученного результата. Высокое значение метрики может быть связано не только с архитектурой модели, но и с особенностями данных, семантической близостью объектов или популярностным смещением.
...
Отдельная линия работ связана с популярностным смещением. Популярные объекты чаще встречаются в данных и могут получать преимущество в метриках даже без сложной персонализации [5]. В современных обзорах популярностное смещение рассматривается как одна из ключевых проблем автономной оценки рекомендательных систем [6, 7]. Поэтому сравнение новой модели только с графовыми базовыми методами может быть недостаточным.
...
Методы причинной и контрфактической оценки рекомендаций дают более глубокий анализ смещений, но часто требуют информации о политике показа, вероятностях экспозиции или пользовательском эксперименте [12, 13]. В рассматриваемой постановке таких данных нет. Поэтому здесь выбрана более узкая задача: сформировать диагностический протокол для автономной оценки, который различает семантическое доминирование, популярностное доминирование, эмпирическое превосходство относительно контролей, смешанный режим и отрицательный перенос.

В российских публикациях последних лет близкие вопросы рассматриваются в нескольких направлениях. В отечественной литературе описаны общие архитектуры рекомендательных систем, включая коллаборативные, контентные и гибридные подходы [14], применение графовых нейронных сетей к рекомендованию товаров в электронной коммерции [15], выбор метрик и процедур оценки качества рекомендаций [16], а также прикладные решения проблем холодного старта, разреженности данных и коррекции релевантности [17, 18]. Для предлагаемого протокола важна именно эта методическая линия: интерес переносится с одного итогового значения метрики на условия, при которых модель дает устойчивый и интерпретируемый результат.
...
Под диагностическим протоколом далее понимается не новая метрика и не замена стандартной экспериментальной оценки, а последовательность проверок, сопровождающих основное сравнение моделей. Для графово-семантических моделей это особенно важно: протокол отделяет ситуацию, когда семантическая ветвь действительно помогает низкосвязным объектам, от случая, когда высокое качество уже достигается без обучения графовой модели. В результате вывод становится осторожнее, но лучше поддается проверке.
...
Несколько доменов нужны для проверки протокола не только в сценарии с ожидаемым положительным результатом. Amazon Books используется как каталог с текстово выраженными свойствами объектов и контролируемой разреженностью пользовательских историй. Сокращенный срез Movies отражает прикладной мультимедийный сценарий, но в проведенных экспериментах не дает положительного эффекта для графово-семантической модели. Сокращенный срез Yelp выбран как случай, где агрегированная метрика чувствительна к популярности объектов. Такое сочетание данных охватывает не только успешный сценарий, но и два типа ограничений: отрицательный перенос и популярностное доминирование.
...
AsymLightGCN используется как графово-семантическая модификация LightGCN. Графовая часть строит пользовательский вектор \(e_u\) и коллаборативный вектор объекта \(g_i\) после \(L\) слоев нормированного распространения. Исходный семантический вектор объекта \(s_i\) проецируется в то же латентное пространство: \(z_i=P_s(s_i)\). В отличие от LightGCN, итоговый объектный вектор смешивает графовую и семантическую части с весом, зависящим от степени объекта \(c_i\) в обучающем графе:
...
Основные параметры запусков приведены ниже. Таблица фиксирует экспериментальный протокол, по которому получены результаты статьи.
...
Все модели сравнивались в едином режиме ранжирования. Для каждого пользователя из выдачи исключались объекты, присутствующие в обучающей части его истории, после чего качество оценивалось по оставшимся кандидатам. Иначе метрика может отражать не способность модели находить новые релевантные рекомендации, а повторное попадание уже наблюдавшихся взаимодействий. Одинаковое разбиение и общий набор кандидатов уменьшают влияние технических деталей оценки на сравнение моделей.

В качестве основных метрик использовались Recall@10, NDCG@10, Recall@50 и NDCG@50. Recall@K показывает долю релевантных тестовых объектов, попавших в первые \(K\) позиций выдачи, а NDCG@K дополнительно учитывает позицию найденных объектов. Поэтому расхождение между Recall и NDCG важно для интерпретации: модель может увеличивать охват релевантных объектов в более широком списке, но не обязательно улучшать верхние позиции. Именно такая ситуация наблюдается на Amazon, минимум 5, где положительный эффект AsymLightGCN по Recall сочетается с сильным семантическим контролем по NDCG@10.
...
Этот случай объясняет, почему сравнения только между обучаемыми моделями недостаточно. Если AsymLightGCN превосходит LightGCN, но простая популярностная стратегия дает более высокие Recall и NDCG, положительный результат относительно одной базовой модели не доказывает качество сам по себе. Для такой структуры данных важнее вопрос, выходит ли модель за пределы популярностной выдачи. По результатам Yelp такой вывод сделать нельзя: здесь прежде всего проявляется популярностное смещение.
...
Сокращенный срез Movies рассматривается как мультимедийный сценарий с отрицательным результатом. В этом срезе семантический контроль практически не работает, а AsymLightGCN уступает LightGCN и BPR-MF. Тем самым фиксируется важная граница применимости: семантическая поддержка не становится автоматически полезной даже в мультимедийном домене.

Отрицательный результат на Movies имеет самостоятельную методическую ценность: наличие мультимедийной предметной области и содержательных признаков не гарантирует полезности семантического вмешательства. Возможны ситуации, когда пользовательские предпочтения лучше восстанавливаются по структуре взаимодействий, чем по доступным описаниям и метаданным. Поэтому прикладная интерпретация графово-семантической модели должна учитывать качество самих признаков, плотность пользовательских историй и соответствие семантической близости реальному поведению пользователей.
...
Сводная диагностическая интерпретация результатов дана ниже.
...
С практической точки зрения протокол работает как предварительный фильтр перед более дорогими экспериментами. Если новая модель не превосходит семантический или популярностный контроль, переход к более сложной оценке требует дополнительного обоснования. Если модель улучшает базовые методы только в отдельных метриках или сегментах, вывод следует формулировать с указанием этих условий. Если же улучшение сохраняется относительно обучаемых моделей и простых контролей, а также подтверждается в целевых сегментах каталога, результат можно считать более обоснованным кандидатом для дальнейшей онлайн- или пользовательской проверки.

Для внедрения графово-семантических моделей из этого следуют несколько практических проверок. До запуска модели необходимо оценить долю редких объектов, качество содержательных признаков и силу простых контрольных стратегий. После обучения следует сопоставлять не только средние метрики, но и поведение в сегментах популярности. При подготовке вывода автономную диагностическую оценку нужно отделять от утверждений о пользовательском эффекте: даже устойчивый offline-результат не доказывает улучшение пользовательского опыта без онлайн-эксперимента или отдельного пользовательского исследования.
